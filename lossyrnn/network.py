...
class EncoderCell(nn.Module):
    def __init__(self):
        ...
        self.rnn1 = ConvLSTMCell(64, 256, kernel_size=3,
            stride=2, padding=1, hidden_kernel_size=1, bias=False)

        self.rnn2 = ConvLSTMCell(256, 512, kernel_size=3,
            stride=2, padding=1, hidden_kernel_size=1, bias=False)

        self.rnn3 = ConvLSTMCell(512, 512, kernel_size=3,
            stride=2, padding=1, hidden_kernel_size=1, bias=False)
...
"""
Binarizer
a binary bottleneck helps force the network to learn efﬁcient representations 
compared to standard ﬂoating-point layers, 
which may have many redundant bit patterns that have no effect on the output.
"""
...
class DecoderCell(nn.Module):
    def __init__(self):
        ...
        self.rnn1 = ConvLSTMCell(512, 512, kernel_size=3,
            stride=1, padding=1, hidden_kernel_size=1, bias=False)

        self.rnn2 = ConvLSTMCell(128, 512, kernel_size=3,
            stride=1, padding=1, hidden_kernel_size=1, bias=False)

        self.rnn3 = ConvLSTMCell(128, 256, kernel_size=3,
            stride=1, padding=1, hidden_kernel_size=3, bias=False)

        self.rnn4 = ConvLSTMCell(64, 128, kernel_size=3,
            stride=1, padding=1, hidden_kernel_size=3, bias=False)
    ...
    def forward(self, input, hidden1, hidden2, hidden3, hidden4):
        ...
        # pixel_shuffle: Rearranges elements in a tensor
